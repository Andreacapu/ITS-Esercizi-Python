...
print("c in esadecimale:", hex(c)[2:])
...
# Converti d in intero
d_int = int(d, 16)

# DECIFRAZIONE
decifrato_int = pow(c, d_int, n_int)

# Converti il risultato decifrato in testo
try:
    # Converti l'intero in bytes
    hex_decifrato = hex(decifrato_int)[2:]
    # Se la lunghezza è dispari, aggiungi uno zero all'inizio
    if len(hex_decifrato) % 2:
        hex_decifrato = '0' + hex_decifrato
    
    bytes_decifrati = bytes.fromhex(hex_decifrato)
    messaggio_originale = bytes_decifrati.decode('utf-8')
    print("Messaggio decifrato:", messaggio_originale)
    
except Exception as e:
    print("Errore nella decodifica:", e)
    print("Valore decifrato (hex):", hex(decifrato_int))
    print("Valore decifrato (int):", decifrato_int)